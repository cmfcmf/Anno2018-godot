...
func draw_field(map, pos, rotation, n_rotations, animation_step):
	var sx = b_size.x if rotation % 2 == 0 else b_size.y
	var sy = b_size.y if rotation % 2 == 0 else b_size.x
	for y in range(sy):
		for x in range(sx):
			map.set_cellv(pos + Vector2(x, y), get_tile_id(x, y, rotation, n_rotations, animation_step))

func get_tile_id(x, y, rotation, n_rotations, animation_step):
	# This is where the first tile for the given rotation is
	var tile_id = 1 + b_gfx + (rotation % n_rotations) * b_size.x * b_size.y + animation_step * n_rotations * b_size.x * b_size.y
